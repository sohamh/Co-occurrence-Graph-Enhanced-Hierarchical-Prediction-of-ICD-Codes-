...
def save_everything(args, metrics_hist_all, model, model_dir, params, criterion, evaluate=False, Best=0):
    """
    ...
    if not evaluate:
        ...
        if not np.all(np.isnan(metrics_hist_all[0][criterion])):
            ...
            if eval_val == len(metrics_hist_all[0][criterion]) - 1:
                ...
                # torch.save(sd, model_dir + "/model_last_%s.pth" % criterion)
                torch.save(sd, model_dir + "/model_best_%s.pth" % criterion)
                # cc=metrics_hist_all[0][args.criterion][0]
                # if(cc>Best):
                #     BEST=cc
                #     torch.save(sd, model_dir + "/model_best_%s.pth" % criterion)
                if args.gpu:
    ...
    print("BEST and Best IS= " + str(BEST) +" "+ str(Best) )
    return BEST