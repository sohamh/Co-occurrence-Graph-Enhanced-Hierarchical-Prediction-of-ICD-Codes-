...
def get_target_level_3(dict,L3_desc,L3,input):
    count = 0
    Y2 = []
    L3_desc[len(L3_desc) + 1] = 'NOT IN THIS LEVEL'
    for k, v in input.items():
        if v in L3:
            Y2.append(L3.index(v))
            count = count + 1
        else:
            Y2.append(len(L3_desc))
    output=list_to_dict(Y2)
    return output

def dict_for_hierarchy(dicts):
    ...
    L1 = [k for k, v in X.items() if v == 'ICD-9']  # Layer1.keys()]
    L2 = [k for k, v in X.items() if v in L1]
    L3 = [k for k, v in X.items() if v in L2]
    L4 = [k for k, v in X.items() if v in L3]
    L1_desc = list_to_dict(L1)  # Layer1.keys()]
    L2_desc = list_to_dict(L2)
    L3_desc = list_to_dict(L3)
    L4_desc = list_to_dict(L4)
    ### for level 3
    Y = [X.get(v.replace('.', '')) for k, v in dicts['ind2c'].items()]
    Y = list_to_dict(Y)
    Y_D=get_target_level_3(dict, L3_desc, L3, Y) #for level 3

    Y2 = [X.get(v) for k, v in Y.items()]
    Y2 = list_to_dict(Y2)
    Y_D2 = get_target_level_3(dict, L2_desc, L2, Y2) #for level 2

    Y3 = [X.get(v) for k, v in Y2.items()]
    Y3 = list_to_dict(Y3)
    Y_D3 = get_target_level_3(dict, L1_desc, L1, Y3)  # for level 1

    return Y_D3, Y_D2, Y_D


def main(args):
    start = time.time()
    args, model, optimizer, scheduler, params, dicts = init(args)
    epochs_trained = train_epochs(args, model, optimizer,scheduler, params, dicts)
...
def init(args):
    ...
    dict_1, dict_2, dict_3 = dict_for_hierarchy(dicts)
    dicts['L1'] = dict_1
    dicts['L2'] = dict_2
    dicts['L3'] = dict_3
    ...
    if(args.from_checkpoint):
        pdict=torch.load(args.checkpoint)
        del pdict['U.weight']
        del pdict['U.bias']
        del pdict['final.weight']
        del pdict['final.bias']
        try:
            del pdict['OA.bias']
            del pdict['OA.weight']
        except:
            print("no ontology attention!")

        cdict=model.state_dict()
        cdict.update(pdict)
        model.load_state_dict(cdict)
    ...
    return args, model, optimizer,scheduler, params, dicts


def train_epochs(args, model, optimizer, scheduler, params, dicts):
    BEST = 0
    """
    ...
    for epoch in range(args.n_epochs):
        ...
        metrics_all = one_epoch(model, optimizer,scheduler, args.Y, epoch, args.n_epochs, args.batch_size, args.data_path,
        ...
        BEST=persistence.save_everything(args, metrics_hist_all, model, model_dir, params, args.criterion, evaluate,BEST)
...
def one_epoch(model, optimizer, scheduler, Y, epoch, n_epochs, batch_size, data_path, version, testing, dicts, model_dir,
              samples, gpu, quiet):
    """
        Wrapper to do a training epoch and test on dev
    """
    if not testing:
        losses, unseen_code_inds = train(model, optimizer, scheduler, Y, epoch, batch_size, data_path, gpu, version, dicts, quiet)
        loss = np.mean(losses)
...
def train(model, optimizer,scheduler, Y, epoch, batch_size, data_path, gpu, version, dicts, quiet):
    """
        Training loop.
